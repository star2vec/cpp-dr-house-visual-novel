...
MedicalActionFactory::MedicalActionFactory() {
...
std::unique_ptr<MedicalAction> MedicalActionFactory::create(const std::string& name) const {
...
int MedicalActionFactory::getCost(const std::string& name) const {
    auto it = actionCosts.find(name);
    return (it != actionCosts.end()) ? it->second : 0;
}

const std::vector<std::pair<std::string, std::string>>& MedicalActionFactory::getAllActions() const {
